...
export class ApiService {
  ...
  getWuwaTasks(): Observable<any> {
    return this.http.get(`${this.baseUrl}/wuwa-tasks`, {
      headers: this.getAuthHeaders(),
    });
  }

  addWuwaTask(taskData: FormData): Observable<TodoItem> {
    return this.http.post<TodoItem>(`${this.baseUrl}/wuwa-tasks`, taskData, {
      headers: this.getAuthHeaders()
    }).pipe(
      catchError((error) => {
  ...
  updateWuwaTaskStatus(id: number, completed: boolean): Observable<any> {
    const status = completed ? 'completed' : 'pending';
    return this.http.put(`${this.baseUrl}/wuwa-tasks/${id}/status`, { status }, {
      headers: this.getAuthHeaders(),
    });
  }

  updateWuwaTaskCategoryAndOrder(id: number, newCategory: string): Observable<any> {
    return this.http.put(`${this.baseUrl}/wuwa-tasks/${id}/category`, { category: newCategory }, {
      headers: this.getAuthHeaders()
    });
  }

  deleteWuwaTask(id: number): Observable<any> {
    return this.http.delete(`${this.baseUrl}/wuwa-tasks/${id}`, {
      headers: this.getAuthHeaders()
    });
  }

  updateWuwaTask(task: { id: number; title: string; description: string }): Observable<any> {
    return this.http.put(`${this.baseUrl}/wuwa-tasks/${task.id}`, {
      title: task.title,
      description: task.description
    }, {
      headers: this.getAuthHeaders()
